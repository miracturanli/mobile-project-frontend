...
export default class AnaYemek extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      FlatListItems: [],
    ...
    db.transaction(tx => {
      tx.executeSql('SELECT * FROM tbl_anayemekler', [], (tx, results) => {
        var temp = [];
    ...
    return(
      <TouchableOpacity style={[styles.itemContainer,{backgroundColor: index % 2 === 1 ? '#E5FFE4' : '#FDE1E1' }]}>
          <Image 
              ...
              source={{uri: item.yemek_photo}}>
          </Image> 
          <View style={styles.textContainer}>
               <Text style={styles.nameStyle}>{item.yemek_name}</Text>
          </View>
      </TouchableOpacity>
    )    
  };
